...
describe('Mockery', () => {
  ...
  class Foo {
    ...
    string = ':-)';
    anyFunction = (param: string): any => param; // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  describe('of', () => {
    it('works with arrays', () => {
      const mock = Mockery.of<Array<Foo>>([
        { string: 'foo' },
        { string: 'bar' }
      ]);
    ...
    it('mocks a promise function', async () => {
      const mock = Mockery.of<Foo>({
        promiseFunction: async () => Promise.resolve({ c: true, d: false })
      });
    ...
    it('works with nested types', () => {
      const mock = Mockery.of<Foo>({
        nestedObject: { stringFunction: () => 'hi' }
      });
    ...
    it('mocks multiple level nested functions', () => {
      const mock = Mockery.of<Foo>({
        nestedObject: { anotherNestedObject: { function: () => false } }
      });
    ...
    it('mocks multiple level nested functions with arrays', () => {
      const mock = Mockery.of<Foo>({
        array: [{ anotherNestedObject: { function: () => false } }]
      });
    ...
    it('does not blow up when a value is null', () => {
      expect(() => {
        Mockery.of<Foo>({ any: null });
      }).not.toThrow(); // tslint:disable-line:no-null-keyword
    });

    it('noops have independent spies', () => {
      const mock = Mockery.of<Foo>({
        nestedObject: { stringFunction: Mockery.noop },
        objectFunction: Mockery.noop
      });
      mock.objectFunction();
  ...
  describe('extend', () => {
    ...
    it('Throws error when trying to extend a mock with circular reference', () => {
      ...
      expect(() => Mockery.extend<Circular>({}).with(myObject)).toThrow(
        new Error(errorMessage)
      );
    });
  });
  ...
  describe('staticMethod', () => {
    ...
    it('is a spy', () => {
      Mockery.staticMethod(Foo, 'static', () => 'hi');
      Foo.static();
      expect(Foo.static).toHaveBeenCalled(); // eslint-disable-line @typescript-eslint/unbound-method
    });
    ...
    it('resets the call count', () => {
      ...
      Foo.static();
      Mockery.staticMethod(Foo, 'static', () => 'hi');
      expect(Foo.static).not.toHaveBeenCalled(); // eslint-disable-line @typescript-eslint/unbound-method
    });
  ...
  describe('all', () => {
    ...
    if (typeof jest !== 'undefined') {
      it('works with indexer', async () => {
        type MapLike = { [key: string]: unknown };

        const mockedData = Mockery.all<MapLike>();
        const x = jest.fn();
        x.mockResolvedValue(mockedData);

        const result = await x(); // eslint-disable-line @typescript-eslint/no-unsafe-assignment
